...
impl<Arrangement, const N: usize> DiscretizedHx<Arrangement, N> {
        ...
        BottomFluid: Clone,
    {
        ...
        let min_delta_t = compute_min_delta_t(&nodes);
...
mod tests {
    ...
    fn counterflow_ua_matches_functional_solver() {
        ...
        let c_dot_top = CapacitanceRate::from_quantity(m_dot_top * model.cp()).unwrap();
        let c_dot_bottom = CapacitanceRate::from_quantity(m_dot_bottom * model.cp()).unwrap();
        ...
        let functional_result = functional::known_conditions_and_inlets(
            &CounterFlow,
            (
                StreamInlet::new(c_dot_top, known.inlets.top.temperature),
