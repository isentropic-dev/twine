...
/// [`Constrained::new`] method or the convenient [`UnitIntervalLeftOpen::new`]
/// associated function.
///
/// # Examples
///
...
///
/// ```
/// use twine_core::constraint::{Constrained, UnitIntervalLeftOpen};
///
/// // Generic constructor:
/// let a = Constrained::<_, UnitIntervalLeftOpen>::new(0.25).unwrap();
...
///
/// // Endpoint:
/// let o = UnitIntervalLeftOpen::one::<f64>();
/// assert_eq!(o.into_inner(), 1.0);
///
/// // Error cases:
/// assert!(UnitIntervalLeftOpen::new(0.0).is_err());
/// assert!(UnitIntervalLeftOpen::new(-0.5).is_err());
...
///
/// let r = Constrained::<Ratio, UnitIntervalLeftOpen>::new(Ratio::new::<ratio>(0.42)).unwrap();
/// assert_eq!(r.as_ref().get::<percent>(), 42.0);
///
/// let o = UnitIntervalLeftOpen::one::<Ratio>();
...
impl UnitIntervalLeftOpen {
    ...
    #[must_use]
    pub fn one<T: UnitBounds>() -> Constrained<T, UnitIntervalLeftOpen> {
        Constrained::<T, UnitIntervalLeftOpen> {
            value: T::one(),
            _marker: PhantomData,
        }
    }
...
mod tests {
    ...
    fn floats_valid() {
        assert!(Constrained::<f64, UnitIntervalLeftOpen>::new(0.1).is_ok());
        assert!(Constrained::<f64, UnitIntervalLeftOpen>::new(1.0).is_ok());
        assert!(UnitIntervalLeftOpen::new(0.75).is_ok());
    ...
    fn floats_out_of_range() {
        ...
        assert!(matches!(
            UnitIntervalLeftOpen::new(-1.0),
            Err(ConstraintError::BelowMinimum)
        ));
        assert!(matches!(
            UnitIntervalLeftOpen::new(1.0000001),
    ...
    #[allow(clippy::float_cmp)]
    fn uom_ratio_valid() {
        assert!(Constrained::<Ratio, UnitIntervalLeftOpen>::new(Ratio::new::<ratio>(0.01)).is_ok());
        assert!(Constrained::<Ratio, UnitIntervalLeftOpen>::new(Ratio::new::<ratio>(1.0)).is_ok());
        assert!(UnitIntervalLeftOpen::new(Ratio::new::<ratio>(0.5)).is_ok());
    ...
    fn uom_ratio_out_of_range() {
        assert!(matches!(
            UnitIntervalLeftOpen::new(Ratio::new::<ratio>(0.0)),
            Err(ConstraintError::BelowMinimum)
        ));
        assert!(matches!(
            UnitIntervalLeftOpen::new(Ratio::new::<ratio>(1.1)),
            Err(ConstraintError::AboveMaximum)
        ));
    }
}
