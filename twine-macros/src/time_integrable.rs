...
impl Parsed {
    ...
    fn generate_derivatives_struct(&self) -> TokenStream {
        let vis = &self.vis;
        let deriv_struct_name = self.ident.with_suffix("Derivative");
    ...
    fn generate_time_integrable_impl(&self) -> TokenStream {
        ...
        quote! {
            impl #impl_generics twine_core::TimeIntegrable for #struct_name #ty_generics #where_clause {
                ...
                fn step(self, derivative: #deriv_struct_name #ty_generics, dt: uom::si::f64::Time) -> Self {
                    Self {
                        #(#step_assignments),*
                    }
                }
...
mod tests {
    ...
    fn generates_correct_code() {
        ...
        let expected_code = quote! {
            #[derive(Debug, Clone, PartialEq)]
            pub struct StateVariablesDerivative {
            ...
            impl twine_core::TimeIntegrable for StateVariables {
                ...
                fn step(self, derivative: StateVariablesDerivative, dt: uom::si::f64::Time) -> Self {
                    Self {
    ...
    fn supports_generics() {
        ...
        let expected_code = quote! {
            #[derive(Debug, Clone, PartialEq)]
            pub(crate) struct StateDerivative<Fluid: TimeIntegrable> {
            ...
            impl<Fluid: TimeIntegrable> twine_core::TimeIntegrable for State<Fluid> {
                ...
                fn step(self, derivative: StateDerivative<Fluid>, dt: uom::si::f64::Time) -> Self {
                    Self {
